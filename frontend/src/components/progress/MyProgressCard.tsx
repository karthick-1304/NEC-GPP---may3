...
import {
  GraduationCap, Mail, Building2, Calendar, Hash,
...
/**
 * Student-only progress view. Backend's /progress/students/:id allows the
 * student themselves through (the controller checks role === 'Student' && id != userId).
 */
export const MyProgressCard = () => {
  const { user } = useAuth();
  const studentId = user?.user_id ?? 0;
  ...
  if (isLoading) return <div className="flex items-center gap-2 text-sm text-slate-500"><Spinner /> Loading…</div>;
  if (error || !data) return <div className="text-sm text-red-600">Could not load your progress.</div>;

  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="card p-6 sm:p-7 grid sm:grid-cols-[auto_1fr] gap-5 items-center">
        <div className="grid h-20 w-20 place-items-center rounded-2xl bg-navy-800 text-white text-2xl font-bold shrink-0">
          {(data.general.full_name?.[0] ?? '?').toUpperCase()}
        </div>
        <div>
          <h2 className="font-display font-bold text-navy-900 text-xl">{data.general.full_name}</h2>
      ...
      <section className="card p-5">
        ...
        {data.top_subjects.length === 0 ? (
          <p className="text-sm text-slate-500">Nothing in the last 2 months — pick a subject and start a set.</p>
        ) : (
      ...
      {/* History */}
      <section className="card p-5">
        <h3 className="text-xs uppercase tracking-wider text-slate-500 font-semibold mb-3">Recent attempts (latest 10)</h3>
        {data.history.length === 0 ? (
          <p className="text-sm text-slate-500">No attempts yet — your first practice will land here.</p>
        ) : (
